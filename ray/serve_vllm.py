...
class VLLMPredictDeployment:
    ...
    async def __call__(self, request: Request) -> Response:
        ...
        conversation = request_dict.pop("messages")
...
def send_sample_request():
    ...
    conversation = [{"role": "user", "content": "给我讲个睡前故事"}]
    sample_input = {"messages": conversation, "stream": True, "max_tokens": 1024}
    output = requests.post("http://localhost:8000/", json=sample_input)
    output_text = ""
    for line in output.iter_lines():
        output_text += json.loads(line.decode("utf-8"))["text"]
    print(output_text)
...
if __name__ == "__main__":
    ...
    # for more details.
    deployment = VLLMPredictDeployment.bind(model="Qwen/Qwen2.5-7B-Instruct", enforce_eager=True)
    serve.shutdown()
    serve.run(deployment)

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(send_sample_request) for _ in range(4)]
